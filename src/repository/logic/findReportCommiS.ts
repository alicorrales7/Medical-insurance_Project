...
export class findReportCommiS{
    ...
    var valueArr = array.map(function(item){ return item.policyNumber });
    ...
    duplicates.forEach(element => {
        relations.push({policyNumber: element, cont: getOccurrence(array, element)})
    });
    return relations;
    }else{
       return [{}]
    ...
    dateResponse(commissionStatementArray:any){
        ...
        let clientsCommon  =  commissionStatementArray.filter(e => {
